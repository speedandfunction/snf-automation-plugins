...
- **Open tasks (current statuses, AFTER Step 2's changes):** **slice the Step-2A pull** — Step 2A is the single owned-task source; do NOT re-query `clickup_filter_tasks` per status (Hard Rule 10), **reconciled with Step 2C's `applied[]`** so a task you just moved in 2C sits in its NEW bucket (making "AFTER Step 2's changes" literally true) — for the **In Progress + To-Do** buckets → a numbered list, each task name a **clickable link** (Hard Rule 9): `№ | task (linked) | priority | status | deadline`. (Blocked is its own step; Closed already in "done".)
- **In Review — ask whether to add to the plan (Andy: "чи треба додавати в план задачі з Рев'ю").** Also list the user's **In Review** tasks (numbered, linked) as *candidates* and let the user pick which to add to today's plan. This is **display-only** — adding a Review task to the report does NOT change its ClickUp status (Hard Rule 2). Fold this into the SAME "which to add" question below (one ask, not a new gate).
...
(Emails, if any, are task-suggestions folded into question 3.) Every task name is a **clickable link** (Hard Rule 9). **The post MUST mirror ONLY what the user picked** in Steps 2/4/5 — never auto-expand to the full task list (Andy: "я вказав дві задачі, а він запостив усі"). If the live Geekbot questions don't obviously match this mapping, show the mapping and let the user confirm/edit before posting.
