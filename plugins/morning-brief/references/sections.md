...
Details for the SKILL.md steps: the status-verb→status mapping + safe apply (Step 2), the "what was done" detection (Step 4), the plate dedup (Step 5), blocker-reason derivation (Step 6), the TeamMD resolver (Step 7/8), the real-Geekbot-mood read (Step 3), and the Geekbot payload (Step 8). Tool params are transcribed from the ClickUp MCP schemas, the Geekbot v1 API, and the team.md export.
...
Resolvers:
- `clickup_id_to_record(id) → {name, slack_id, email}` — for reviewer resolution (Step 4B) from a comment's `assignee.id`.
...
3. **Render the exact payload and PREVIEW it** (Hard Rule 4):
   ...
   ```
   `answers` keys = question ids as STRINGS; ALL of the standup's questions must be present (empty-but-required → a short "—"). The mood question's answer = the chosen real option string (Step 3) — **byte-for-byte one of that question's live `answer_choices` strings, never a paraphrase or free-text** (S8).
