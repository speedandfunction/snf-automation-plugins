...
- **Open tasks:** `clickup_filter_tasks(assignees=[me], statuses=["in progress","to do"], include_closed=false)` → a **flat-numbered** list, renumbered into the brief's single continuous sequence at render (see Step 7; numbers let the user say "drop 3"). Print the ClickUp rows FIRST (fast).
...
## Step 7 — Review & confirm (the user edits BOTH "what was done" and "on your plate")

Assemble the brief in this order — **Done · On your plate · Blockers · Open questions · (Emails)** — TZ + window in a footer. **Number EVERY editable row in one continuous sequence across the Done status-changes AND the On-your-plate tasks AND the not-yet-ticketed call items** (meetings stay GROUPED under their own heading — not individually numbered, per Step 2A). The numbers let the user act on any row. **Blockers are shown for awareness but stay OUTSIDE the editable number run; editing a Done row is display-only — it changes the brief/Geekbot text, NEVER writes back a ClickUp status.**

Then **present the whole brief and explicitly confirm it before anything is finalized/posted** — Andy wants to *agree what was done* and *trim the plate* (transcript: "узгоджує… що було виконано, питає так/не так, добавити щось чи забрати" for Done; plate items "нумеровані, щоб можна було видалити зайві"). So this is a real review gate, not just a print. Accept edit-by-exception on ANY section (Done included):
- `drop <n>` — remove a row (a "done" item you didn't actually do, or an irrelevant plate task)
- `add <text>` — add a missed done-item or plate task (work not captured from ClickUp/calls)
- `edit <n>: <text>` — reword a row
- `done` / `ok` — accept the brief as shown

Reprint after each edit. A bad/ambiguous ref → say so and reprint, never guess. **Nothing is finalized — and Step 8 never posts — until the user accepts.**
